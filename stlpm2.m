...
XR=[ones(length(XR),1) XR max(E(:,1),0) min(E(:,1),0)];   % construct common RHS projection matrix
if includetrend==1
    XR=[trend XR];
end
XRproj=inv(XR'*XR)*XR';
...
if scaling==1
    ...
    results(N+1).NL.scaled.coef(:,2)=betafull(:,end)/betafull(1,end); % Scale by first period impact on Fed Funds
    ...
    results(N+1).NL.smoothed.coef(:,2)=weight*betafull(:,end)/betafull(1,end); % Scale by first period impact on Fed Funds
    ...
    results(N+1).NL.cum.coef(:,2)=cumweight*betafull(:,end)/betafull(1,end);
...
if scaling==1
    ...
    results(n).NL.scaled.coef(:,2)=-results(n).NL.betafull(:,end)/results(N+1).NL.betafull(1,end); % Scale by first period impact on Fed Funds
    ...
    results(n).NL.smoothed.coef(:,2)=-weight*results(n).NL.betafull(:,end)/results(N+1).NL.betafull(1,end); % Scale by first period impact on Fed Funds
    ...
    results(n).NL.cum.coef(:,2)=-cumweight*results(n).NL.betafull(:,end)/results(N+1).NL.betafull(1,end);
...
%         results(n).lin.scaled.coef=results(n).lin.betafull/betaR(1); % Scale by first period impact on Fed Funds
